...
abigen!(
    PythContract,
    r#"[ function getUpdateFee(bytes[]) view returns (uint256) ]"#,
);
...
pub fn b64_to_hex(input: &str) -> Result<String> {
    ...
    let hex_string: String = decoded_bytes.iter().map(|b| format!("{:02x}", b)).collect();
...
pub async fn fetch_testnet_vaas(addresses: Vec<&str>) -> Result<Vec<Bytes>> {
    ...
    let pyth_vaas: Vec<Bytes> = reqwest::get(format!(
        "https://xc-testnet.pyth.network/api/latest_vaas?ids[]={}",
        addresses
    ))
    .await?
    .json::<Vec<String>>()
    .await?
    .iter()
    .map(|x| base64::decode(x).unwrap().into())
    .collect();
    Ok(pyth_vaas)
}
